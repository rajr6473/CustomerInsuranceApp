...
// Eventually replace hardcoded samplePolicies with fetched data
  // useEffect(() => {
  //   api.getUserDashboard(auth.getToken())
  //      .then(d => setPolicies(d.myInsurance)).catch(() => {});
  // }, []);

useEffect(()=>{ api.getUserDashboard(auth.getToken()).then(d=>setPolicies(d.myInsurance)).catch(()=>{}) },[]);


const onDownload = (item) => { /* call api.downloadPolicyDocument */ }


// return (
// <SafeAreaView style={{flex:1, padding:16}}>
// <Text style={{fontSize:20,fontWeight:'700', marginBottom:12}}>My Insurance</Text>
// <FlatList data={policies} keyExtractor={i=>i.id} renderItem={({item})=> <PolicyItem item={item} onDownload={onDownload} />} />
// </SafeAreaView>
// )
return (
    <SafeAreaView style={{ flex: 1, padding: 16, backgroundColor: '#f5f8fc' }}>
      <Text style={{ fontSize: 20, fontWeight: '700', marginBottom: 12 }}>My Insurance Portfolio</Text>
      <FlatList
        data={policies}
